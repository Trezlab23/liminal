...
export default async function handler(req, res) {
  ...
  try {
    // GET — fetch lesson history and topic progress
    if (req.method === "GET") {
      ...
      // Fetch recent lessons (last 50)
      const lessonsResult = await pool.query(
        `SELECT id, topic_id, title, hook, badge, quiz_correct, xp_earned, duration, created_at
         FROM lessons WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50`,
        [userId]
      );

      // Fetch topic progress (count + total XP per topic)
